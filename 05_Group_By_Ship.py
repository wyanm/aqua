...
handler = RotatingFileHandler('log/05_Group_By_Ship.log', maxBytes=1000000, backupCount=1)
...
def group_data():
    ...
    os.system(f"rm -rf grouped_data/*")
    df=pd.read_parquet("joined_data")# FILTER SET AT JOIN NOW,filters=[('type', '>=', 80),('type', '<', 90)])
    ...
    pq.write_to_dataset(table,root_path='grouped_data', schema=schema_group,partition_cols=['type'])
