...
handler = RotatingFileHandler('log/04_Join_Prepare.log', maxBytes=1000000, backupCount=1)
...
def join_data():
    ...
    os.system(f"rm -rf joined_data/*")
    for x in os.walk("agg_position_reports"):
        if "hour" not in x[0]:
            continue
        logger.info(x[0])
        ...
        pq.write_to_dataset(table,root_path='joined_data', schema=schema_join,partition_cols=['year','month','day','hour'])
