...
handler = RotatingFileHandler('/app/log/02.log', maxBytes=1000000, backupCount=1)
...
def agg_data():
    ...
    for x in os.walk("data/position_reports"):
        if "hour" not in x[0]:
            continue
        ...
        pq.write_to_dataset(table,root_path='data/agg_position_reports', schema=schema_pos_agg,partition_cols=['year','month','day','hour'])
