...
handler = RotatingFileHandler('log/03.log', maxBytes=1000000, backupCount=1)
...
def agg_data():
    ...
    for x in os.walk("static_ship_data"):
        if "hour" not in x[0]:
            continue
        ...
        pq.write_to_dataset(table,root_path='agg_static_ship_data', schema=schema_static_agg,partition_cols=['year','month','day','hour'])
